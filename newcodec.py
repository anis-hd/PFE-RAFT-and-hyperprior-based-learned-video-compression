...
def decode_video_main(config: CodecConfig):
    ...
    latest_decoded_iframe_tensor = None # For low motion replacement
    ...
    try:
        with open(input_rdvc_file_p, 'rb') as rdvc_file:
            ...
            original_fps = video_metadata.get('original_fps', 30.0) # Default if missing
            total_frames_processed = video_metadata['total_frames_processed']
            iframe_format_from_meta = video_metadata.get("iframe_format", "JPEG") # Default if missing
            iframe_jpeg_quality = video_metadata.get("iframe_jpeg_quality", "N/A")
            ...
            print(f"  I-Frame Format (from RDVC metadata): {iframe_format_from_meta}" + (f" (Quality: {iframe_jpeg_quality})" if iframe_format_from_meta=="JPEG" else ""))
            if 'flow_compression_resolution_wh' in video_metadata: print(f"  Encoded Flow Comp. Res (WxH): {video_metadata['flow_compression_resolution_wh']}")
            if 'residual_compression_resolution_wh' in video_metadata: print(f"  Encoded Residual Comp. Res (WxH): {video_metadata['residual_compression_resolution_wh']}")
            if 'raft_source_used' in video_metadata: print(f"  RAFT Source Used (Encoding): {video_metadata['raft_source_used']}")
            if 'raft_checkpoint_info' in video_metadata: print(f"  RAFT Checkpoint Info (Encoding): {video_metadata['raft_checkpoint_info']}")


            if video_metadata.get('codec_checkpoint_filename', 'N/A') != Path(config.codec_checkpoint_path).name:
                print(f"  WARNING: Codec checkpoint specified in config ('{Path(config.codec_checkpoint_path).name}') "
                      f"differs from RDVC metadata ('{video_metadata.get('codec_checkpoint_filename', 'N/A')}'). Using config path.")
            ...
            print("\n--- Loading Video Codec Model (parameters should align with training/encoding) ---")
            model_codec = VideoCodec( # Use parameters from config
                motion_latent_channels=config.motion_latent_channels,
                residual_latent_channels=config.residual_latent_channels,
                mcn_base_channels=config.mcn_base_channels,
            ...
            if not load_model_checkpoint(config.codec_checkpoint_path, model_codec, "VideoCodec", device, strict_load=False): # strict=False is common for inference models
                raise RuntimeError(f"Failed to load VideoCodec checkpoint: {config.codec_checkpoint_path}")
            print("Video Codec model loaded.")
            ...
            previous_frame_tensor = None # Stores the previously reconstructed frame tensor
            ...
            for expected_frame_idx in pbar:
                ...
                frame_type_char = rdvc_file.read(1) # 'I' or 'P'
                frame_content_len = struct.unpack(UINT64_FORMAT, rdvc_file.read(8))[0]
                
                if stored_frame_idx != expected_frame_idx: # Sanity check
                ...
                if len(frame_content_bytes) != frame_content_len:
                    raise EOFError(f"Could not read full frame content for frame {stored_frame_idx}. "
                                   f"Expected {frame_content_len}, got {len(frame_content_bytes)}.")

                content_io = io.BytesIO(frame_content_bytes) # Use BytesIO for easier parsing of content
                save_debug = (debug_output_path_decode and 
                              stored_frame_idx % config.debug_frame_interval_decode == 0)
                current_reconstructed_frame_tensor = None # Initialize for this frame
                ...
                if frame_type_char == b'I':
                    ...
                    iframe_ext = content_io.read(iframe_ext_len).decode('utf-8') # e.g., ".jpg"
                    iframe_data_bytes = content_io.read() # Remainder is image data

                    current_reconstructed_frame_tensor = load_image_as_tensor(iframe_data_bytes, device, is_bytes=True)
                    if current_reconstructed_frame_tensor is None: 
                        print(f"ERROR: Failed to load I-Frame {stored_frame_idx} data from RDVC. Stopping.")
                        decoding_error_occurred=True; break
                    
                    latest_decoded_iframe_tensor = current_reconstructed_frame_tensor.detach().clone() # Store for P-frame replacement

                    if save_debug: 
                        dbg_pfx = debug_output_path_decode / f"frame_{stored_frame_idx:06d}"
                        save_tensor_as_image_vis(current_reconstructed_frame_tensor, 
                                                 dbg_pfx.with_name(f"{dbg_pfx.name}_iframe_decoded{iframe_ext}"))
                
                # --- P-Frame Decoding ---
                elif frame_type_char == b'P':
                    pbar.set_postfix_str(f"P-Frame {stored_frame_idx}")
                    if previous_frame_tensor is None: 
                        print(f"ERROR: No previous frame available for P-Frame {stored_frame_idx}. This indicates a stream error. Stopping.")
                        decoding_error_occurred=True; break
                    
                    # Initialize P-frame specific return values from decompressor to None
                    flow_rec_upscaled, warped_prev, mcn_out, res_rec_upscaled = None, None, None, None
                    try:
                        # Parse P-frame payload
                        ...
                        if len(motion_bs) != motion_len or len(residual_bs) != residual_len:
                            raise ValueError("P-Frame bitstream length mismatch during RDVC read.")

                        motion_data = (motion_bs, (motion_h, motion_w)) # (bitstream, latent_shape_hw)
                        res_data = (residual_bs, (residual_h, residual_w))
                        
                        # Decompress using the model
                        (current_reconstructed_frame_tensor, flow_rec_upscaled, 
                         warped_prev, mcn_out, res_rec_upscaled) = \
                            model_codec.decompress_frame_simplified(
                                previous_frame_tensor,
                                motion_data,
                                res_data,
                                (original_height, original_width) # Target H,W for outputs
                            )
                        
                        # Initial debug saves for P-frame components (before low-motion replacement)
                        if save_debug:
                            dbg_pfx = debug_output_path_decode / f"frame_{stored_frame_idx:06d}"
                            save_tensor_as_image_vis(previous_frame_tensor, dbg_pfx.with_name(f"{dbg_pfx.name}_00_prev_ref.png"))
                            if flow_rec_upscaled is not None: visualize_flow_hsv(flow_rec_upscaled, dbg_pfx.with_name(f"{dbg_pfx.name}_01_flow_reconstructed.png"))
                            if warped_prev is not None: save_tensor_as_image_vis(warped_prev, dbg_pfx.with_name(f"{dbg_pfx.name}_02_warped_prev.png"))
                            if mcn_out is not None: save_tensor_as_image_vis(mcn_out, dbg_pfx.with_name(f"{dbg_pfx.name}_03_mc_prediction.png"))
                            if res_rec_upscaled is not None: save_tensor_as_image_vis(res_rec_upscaled, dbg_pfx.with_name(f"{dbg_pfx.name}_04_residual_reconstructed.png"), drange=(-0.5,0.5))
                            if current_reconstructed_frame_tensor is not None:
                                save_tensor_as_image_vis(current_reconstructed_frame_tensor, dbg_pfx.with_name(f"{dbg_pfx.name}_05_reconstructed_raw.png"))
                        
                        # --- Low Motion Region Replacement ---
                        if config.low_motion_replacement_threshold > 0.0 and \
                           ...
                           current_reconstructed_frame_tensor is not None:
                            
                            if latest_decoded_iframe_tensor.shape == current_reconstructed_frame_tensor.shape:
                                # Flow magnitude: sqrt(dx^2 + dy^2) -> (B, 1, H, W)
                                flow_magnitude = torch.sqrt(
                                    flow_rec_upscaled[:, 0:1]**2 + flow_rec_upscaled[:, 1:2]**2
                                )
                                # Mask for low motion regions (B, 1, H, W), boolean
                                low_motion_mask = flow_magnitude < config.low_motion_replacement_threshold
                                
                                # Expand mask to match frame channels for torch.where
                                low_motion_mask_expanded = low_motion_mask.expand_as(current_reconstructed_frame_tensor)
                                
                                num_replaced_pixels = torch.sum(low_motion_mask).item() # Count true values in B,1,H,W mask
                                if num_replaced_pixels > 0:
                                    pbar.set_postfix_str(f"P-Frame {stored_frame_idx} (replacing {num_replaced_pixels}px low motion)")
                                
                                # Replace regions
                                current_reconstructed_frame_tensor = torch.where(
                                    low_motion_mask_expanded,
                                    latest_decoded_iframe_tensor, # Content from I-Frame
                                    current_reconstructed_frame_tensor # Content from P-Frame reconstruction
                                )
                                
                                if save_debug and num_replaced_pixels > 0: # Save mask only if replacement happened & debug on
                                    dbg_pfx = debug_output_path_decode / f"frame_{stored_frame_idx:06d}"
                                    save_tensor_as_image_vis(low_motion_mask.float(), # Visualize the B,1,H,W mask
                                                             dbg_pfx.with_name(f"{dbg_pfx.name}_06_low_motion_mask.png"))
                            else:
                                print(f"  Warning: Skipping low motion replacement for P-frame {stored_frame_idx}. "
                                      f"Shape mismatch: current {current_reconstructed_frame_tensor.shape}, I-frame {latest_decoded_iframe_tensor.shape}.")
                        
                        # Save the final version of the P-frame (after potential replacement) for debugging
                        if save_debug and current_reconstructed_frame_tensor is not None:
                            dbg_pfx = debug_output_path_decode / f"frame_{stored_frame_idx:06d}" # Get pfx again just in case
                            save_tensor_as_image_vis(current_reconstructed_frame_tensor, dbg_pfx.with_name(f"{dbg_pfx.name}_07_final_to_video.png"))
                    ...
                    print(f"ERROR: Unknown frame type '{frame_type_char.decode()}' encountered for frame {stored_frame_idx}. Stopping.")
                    decoding_error_occurred=True; break
                
                # --- Write Reconstructed Frame to Video and Update Reference ---
                if current_reconstructed_frame_tensor is not None:
                    frame_to_write_list = tensor_to_cv2_bgr(current_reconstructed_frame_tensor)
                    if frame_to_write_list: # List contains one BGR image
                        video_writer.write(frame_to_write_list[0]) 
                    else: # Should not happen if tensor_to_cv2_bgr is robust
                        print(f"Error: tensor_to_cv2_bgr failed for frame {stored_frame_idx}. Cannot write to video.");
                        decoding_error_occurred=True; break
                    
                    previous_frame_tensor = current_reconstructed_frame_tensor.detach().clone() # Update for next P-frame
                else: 
                    # This implies an error in I-Frame loading or P-Frame decompression that wasn't caught by a break
                    print(f"Error: Reconstructed tensor is None for frame {stored_frame_idx}, cannot write or use as reference. Stopping.");
                    decoding_error_occurred=True; break
                
                # Optional: Clear CUDA cache periodically to manage memory for long videos
                if device.type == 'cuda' and stored_frame_idx > 0 and stored_frame_idx % 50 == 0: # Every 50 frames
                    torch.cuda.empty_cache() 
            
            # --- After Loop: Check for EOF Marker if not already encountered ---
            if not decoding_error_occurred and marker != RDVC_EOF_MARKER: # If loop finished due to range(total_frames)
                ...
                elif not final_marker_check and expected_frame_idx + 1 == total_frames_processed : # Reached end of file as expected
                    pass # This is fine if all frames were processed
                elif not final_marker_check: # File ended prematurely
                     print(f"Warning: RDVC file ended without EOF marker. Processed {pbar.n}/{total_frames_processed} frames based on counter.")


    except ValueError as ve: # Catch RDVC format errors
        print(f"\nVALUE ERROR (likely RDVC format issue): {ve}"); traceback.print_exc(); decoding_error_occurred = True
    except EOFError as eofe: # Catch premature end-of-file during content read
        print(f"\nEOF ERROR (likely truncated RDVC file or content length mismatch): {eofe}"); traceback.print_exc(); decoding_error_occurred = True
    except Exception as loop_err: # Catch any other unexpected errors
        ...
        print(status_msg)
        if debug_output_path_decode and any(debug_output_path_decode.iterdir()): # Check if any debug files were made
...
if __name__ == "__main__":
    ...
    parser.add_argument('--raft_backend', type=str, choices=['auto', 'torchvision', 'local'], default=None, # Default to None to use CodecConfig's default
                        help="Which RAFT implementation to use for encoding: "
                             "'auto' (try torchvision new->old->local), "
                             "'torchvision' (force torchvision raft_large, uses default weights unless raft_checkpoint_path given), "
                             "or 'local' (force local 'core' RAFT, requires raft_checkpoint_path or checkpoint in raft_checkpoint_dir). "
                             "Default: uses CodecConfig default (e.g., 'auto').")
    # Add more CLI arguments here to override CodecConfig defaults if needed.
    # For example:
    # parser.add_argument('--input', type=str, help="Override input file path from CodecConfig.")
    # parser.add_argument('--output', type=str, help="Override output file path from CodecConfig.")
    # parser.add_argument('--codec_checkpoint', type=str, help="Override codec checkpoint path.")
    # parser.add_argument('--low_motion_threshold', type=float, help="Override low motion replacement threshold for decoder.")


    args = parser.parse_args()
    
    config = CodecConfig() # Initialize with defaults
    # Update config from args. This allows args to override defaults.
    # The update_from_args method handles 'gpu' and 'raft_backend' specifically.
    ...
    if args.mode == 'encode':
        ...
        # YUV specific checks (if input is YUV)
        if input_file_for_check.suffix.lower() == ".yuv":
            if not all([config.input_yuv_width, config.input_yuv_height, 
                        config.input_yuv_fps, config.input_yuv_pixel_format]):
                print("FATAL ERROR: YUV parameters (width, height, fps, pixel_format) in CodecConfig are not "
                      "fully and correctly set for the .yuv input file.")
                sys.exit(1)
            if config.input_yuv_pixel_format != "yuv420p": # Check against supported formats in reader
                 print(f"FATAL ERROR: Unsupported input_yuv_pixel_format in CodecConfig: '{config.input_yuv_pixel_format}'. "
                       "Currently, only 'yuv420p' is directly supported by the YUV reader.")
                 sys.exit(1)

        # RAFT checkpoint validation is handled dynamically inside encode_video_main based on selected backend
        # Ensure output directory for RDVC file exists
        ...
        if not input_rdvc_file_for_check.is_file():
            print(f"FATAL: Input RDVC file '{input_rdvc_file_for_check.resolve()}' (from CodecConfig) not found or invalid.")
            sys.exit(1)
        
        # Ensure output directory for decoded video exists
        Path(config.output_video_path_decode).parent.mkdir(parents=True, exist_ok=True)
        if config.debug_frames_dir_decode: # Ensure debug dir exists if specified
            Path(config.debug_frames_dir_decode).mkdir(parents=True, exist_ok=True)

        decode_video_main(config)
    else:
        # This case should not be reached due to 'choices' in add_argument, but as a fallback:
        parser.print_help()
        sys.exit(1)
